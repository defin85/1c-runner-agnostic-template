...
- **AND** compact summary ДОЛЖЕН маршрутизировать к project-owned architecture map и runtime quick reference как к следующему слою curated truth, а не только к raw inventory
...
#### Scenario: Curated project-owned truth becomes stale

- **WHEN** project-owned architecture map, runtime quick reference, project map, or advertised project-specific baseline extension references missing paths or inconsistent entrypoints
- **THEN** the semantic checks MUST fail before the generated repo is presented as agent-ready
- **AND** the failure MUST identify which curated truth file drifted
