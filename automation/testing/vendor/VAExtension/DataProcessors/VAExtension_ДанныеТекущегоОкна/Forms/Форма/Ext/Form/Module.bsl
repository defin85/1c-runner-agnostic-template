...
Процедура ВывестиДанные(Команда)
	...
	Если НЕ ОкноНайдено И ВызыватьИсключение Тогда
...
&НаКлиенте
Процедура УстановитьСвойствоОбновлениеТекстаРедактирования(Команда)
	...
	Для Каждого ТекОкно Из Окна Цикл
		Если ТекОкно.Заголовок = ЗначениеЗаголовокОкна Тогда
			Если ТекОкно.Содержимое.Количество() > 0 Тогда
				ОкноНайдено = Истина;
				УстановитьСвойствоОбновлениеТекстаРедактированияДляОкна(ТекОкно);
				Прервать;
			КонецЕсли;	 
		КонецЕсли;	 
	КонецЦикла;
...
&НаКлиенте
Процедура УстановитьСвойствоОбновлениеТекстаРедактированияДляОкна(ТекОкно)
	
	НадоПроверитьЧтоЭлементНайден = Ложь;
	ЭлементНайден = Ложь;
	Если ЗначениеЗаполнено(ОтборИмяПоля) Тогда
		НадоПроверитьЧтоЭлементНайден = Истина;
	КонецЕсли;	
	
	Для Каждого ТекФорма Из ТекОкно.Содержимое Цикл
		Для Каждого ТекЭлемент Из ТекФорма.Элементы Цикл
			
			Если ЗначениеЗаполнено(ОтборИмяПоля) Тогда
				Если ИскатьПолеПоИмени Тогда
					Если НРег(ТекЭлемент.Имя) <> НРег(ОтборИмяПоля) Тогда
						Продолжить;
					Иначе	
						ЭлементНайден = Истина;
					КонецЕсли;
				Иначе	
					Если НРег(ТекЭлемент.Заголовок) <> НРег(ОтборИмяПоля) Тогда
						Продолжить;
					Иначе	
						ЭлементНайден = Истина;
					КонецЕсли;
			...
			ИначеЕсли ТипЭлемента = Тип("ПолеФормы") Тогда
				Если ТекЭлемент.Вид = ВидПоляФормы.ПолеКартинки Тогда
					Продолжить;
				ИначеЕсли ТекЭлемент.Вид = ВидПоляФормы.ПолеHTMLДокумента Тогда
					Продолжить;
				ИначеЕсли ТекЭлемент.Вид = ВидПоляФормы.ПолеФлажка Тогда
					Продолжить;
				ИначеЕсли ТекЭлемент.Вид = ВидПоляФормы.ПолеПереключателя Тогда
					Продолжить;
				ИначеЕсли ТекЭлемент.Вид = ВидПоляФормы.ПолеТекстовогоДокумента Тогда
					Продолжить;
				ИначеЕсли ТекЭлемент.Вид = ВидПоляФормы.ПолеПолосыРегулирования Тогда
					Продолжить;
				ИначеЕсли ТекЭлемент.Вид = ВидПоляФормы.ПолеНадписи Тогда
					Продолжить;
				КонецЕсли;	
			...
			Попытка
				ТекЭлемент.ОбновлениеТекстаРедактирования = ОбновлениеТекстаРедактирования.ПриИзмененииЗначения;
			Исключение
				//Сообщить(ТипЗнч(ТекЭлемент));
				//Попытка
				//	Сообщить("Вид: " + ТекЭлемент.Вид);
				//Исключение
				//КонецПопытки;
			КонецПопытки;
	...
	Если НадоПроверитьЧтоЭлементНайден И НЕ ЭлементНайден Тогда
		ТекстОшибки = "Не найден элемент <%1>";
		ТекстОшибки = СтрЗаменить(ТекстОшибки, "%1", ОтборИмяПоля);
