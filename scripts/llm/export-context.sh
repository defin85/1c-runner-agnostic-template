...
emit_source_repo_files() {
  ...
  if git -C "$root" rev-parse --is-inside-work-tree >/dev/null 2>&1; then
    for path in "${source_repo_inventory_roots[@]}"; do
      git -C "$root" -c core.quotePath=false ls-files --cached --others --exclude-standard -- "$path"
...
render_generated_metadata() {
...

  {
    ...
    printf '    "workItemsGuide": "docs/work-items/README.md",\n'
    printf '    "workItemsTemplate": "docs/work-items/TEMPLATE.md",\n'
...
render_generated_hotspots_summary() {
...

  {
    ...
    printf -- '- Long-running companion workspace: `docs/work-items/README.md`\n'
