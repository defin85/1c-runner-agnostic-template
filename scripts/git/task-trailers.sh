...
Commands:
  ...
  select-commits [--repo <path>] (--work-item <id> | --range <revset>)
EOF
}
...
validate_message_command() {
  ...
  if [ "$require_any" -eq 1 ] && [ "$work_item_count" -eq 0 ]; then
...
render_command() {
  ...
  [ -n "$work_item" ] || die "render requires --work-item"
...
select_commits_command() {
  ...
  [ -n "$selector_mode" ] || die "select-commits requires one of --work-item or --range"
