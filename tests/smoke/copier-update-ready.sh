...
assert_jq "$rendered_root/automation/context/runtime-support-matrix.json" '.matrixRole == "project-owned-runtime-support-matrix" and (.statuses | sort) == ["operator-local","provisioned","supported","unsupported"] and ([.contours[].id] | sort) == ["agent-verify","bdd","bdd-warm-service","check-cfe-applicability","check-cfe-config","check-x11-contour","codex-onboard","configure-cfe-runtime-flags","doctor","export-context-check","golden-baseline","load-cfe","load-diff-src","load-task-src","publish-http","smoke","web-client-diagnostic","xunit","yaxunit","yaxunit-warm-rpc"] and .projectSpecificBaselineExtension == null and (.contours[] | select(.id == "doctor") | .runbookPath) == "docs/agent/operator-local-runbook.md" and (.contours[] | select(.id == "load-diff-src") | .runbookPath) == "docs/agent/operator-local-runbook.md" and (.contours[] | select(.id == "load-task-src") | .runbookPath) == "docs/agent/operator-local-runbook.md" and (.contours[] | select(.id == "xunit") | .status) == "operator-local" and (.contours[] | select(.id == "xunit") | .runbookPath) == "docs/testing/xunit-direct-platform.md" and (.contours[] | select(.id == "yaxunit-warm-rpc") | .status) == "operator-local" and (.contours[] | select(.id == "bdd-warm-service") | .status) == "operator-local" and (.contours[] | select(.id == "web-client-diagnostic") | .runbookPath) == "docs/agent/operator-local-runbook.md" and (.contours[] | select(.id == "golden-baseline") | .runbookPath) == "tests/golden/README.md"' "generated-runtime-support-matrix"
