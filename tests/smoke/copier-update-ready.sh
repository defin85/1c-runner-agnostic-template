...
assert_exists "$rendered_root/scripts/platform/manage-cfe.sh"
assert_exists "$rendered_root/scripts/platform/v8unpack.sh"
...
assert_exists "$rendered_root/scripts/test/run-bdd-warm-run.sh"
assert_exists "$rendered_root/scripts/test/run-bdd-warm-service.sh"
assert_exists "$rendered_root/scripts/tools/onec-test-port-lease"
...
assert_exists "$rendered_root/automation/context/templates/generated-project-operator-local-targets.json"
...
assert_exists "$rendered_root/tests/smoke/manage-cfe-contract.sh"
assert_exists "$rendered_root/tests/smoke/v8unpack-contract.sh"
...
assert_exists "$rendered_root/tests/smoke/vanessa-bdd-warm-service-contract.sh"
assert_exists "$rendered_root/tests/golden/README.md"
assert_exists "$rendered_root/tests/golden/create.sh"
assert_exists "$rendered_root/tests/golden/restore.sh"
assert_exists "$rendered_root/tests/golden/db.sh"
...
assert_contains "$rendered_root/Makefile" "manage-cfe:"
assert_contains "$rendered_root/Makefile" "v8unpack:"
...
assert_contains "$rendered_root/Makefile" "golden-create:"
assert_contains "$rendered_root/Makefile" "golden-restore:"
assert_contains "$rendered_root/Makefile" "golden-baseline:"
assert_contains "$rendered_root/Makefile" "bdd-warm-service:"
...
assert_contains "$rendered_root/.gitignore" "!.codex/skills/"
...
assert_jq "$rendered_root/env/local.example.json" '.schemaVersion == 3 and (has("runnerAdapter") | not) and (has("transport") | not) and .ibcmd.runtimeMode == "file-infobase" and .ibcmd.serverAccess.mode == "data-dir" and .capabilities.loadSrc.driver == "ibcmd"' "local-example-driver"
assert_jq "$rendered_root/env/ci.example.json" '.schemaVersion == 3 and (has("runnerAdapter") | not) and (has("transport") | not) and .ibcmd.runtimeMode == "dbms-infobase" and .ibcmd.dbmsInfobase.kind == "PostgreSQL" and .capabilities.loadSrc.driver == "designer"' "ci-example-driver"
assert_jq "$rendered_root/env/wsl.example.json" '.schemaVersion == 3 and (has("runnerAdapter") | not) and (has("transport") | not) and .ibcmd.runtimeMode == "standalone-server" and .platform.xvfb.enabled == true and .platform.xvfb.serverArgs == ["-screen","0","1440x900x24","-noreset"] and .platform.ldPreload.enabled == true and .platform.ldPreload.libraries == ["/usr/lib/libstdc++.so.6","/usr/lib/libgcc_s.so.1"] and .capabilities.loadSrc.driver == "designer"' "wsl-example-driver"
assert_jq "$rendered_root/env/windows-executor.example.json" '.schemaVersion == 3 and (has("runnerAdapter") | not) and .transport.kind == "remote-windows" and .capabilities.loadSrc.driver == "designer"' "windows-example-driver"
...
assert_jq "$rendered_root/automation/context/operator-local-targets.json" '.operatorLocalTargets.vanessaBdd.targetId == ""' "generated-operator-local-targets"
...
assert_contains "$rendered_root/automation/context/template-managed-paths.txt" "automation/context/templates/generated-project-operator-local-targets.json"
...
printf '%s\n' 'local-private-secret-marker' >"$rendered_root/env/.local/private.json"
...
assert_contains "$rendered_root/env/.local/private.json" "local-private-secret-marker"
